...
def create_timeslice_data(data: pd.DataFrame, node_data: pd.DataFrame, start_time: int, travers_seconds: int):
    ...
    while start_time >= data['timestamp'].values.min():
        ...
        try:
            sliced_node_data_df.loc[:, 'daily_disk_free'] = sliced_node_data_df.groupby(['destinations', 'layer', 'public_port'])['disk_free'].transform('max')
            sliced_node_data_df.loc[:, 'daily_disk_total'] = sliced_node_data_df.groupby(['destinations', 'layer', 'public_port'])['disk_total'].transform('max')
            sliced_node_data_df.loc[:, 'daily_cpu_count'] = sliced_node_data_df.groupby(['destinations', 'layer', 'public_port'])['cpu_count'].transform('max')
