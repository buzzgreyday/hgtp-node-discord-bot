...
def setup(bot):
    @bot.slash_command(name="verify", description="Verify your server settings to gain access",
                       guild_ids=[974431346850140201], dm_permission=True)
                    ...
                    for cluster_name in _configuration["modules"].keys():
                        for layer in _configuration["modules"][cluster_name].keys():
                            fut.append(asyncio.create_task(
                                main.main(ctx, process_msg, requester, cluster_name, layer, _configuration)))
                    for task in fut:
                        await task
                else:
