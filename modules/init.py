...
async def check(dask_client, process_msg, requester: str, subscriber: dict, layer: int, port: int, latest_tessellation_version: str, all_supported_clusters_data: list[dict], history_dataframe, dt_start, configuration: dict) -> tuple:
    process_msg = await discord.update_proces_msg(process_msg, 2, None)
    ...
    process_msg = await discord.update_proces_msg(process_msg, 3, None)
...
async def process(dask_client, process_msg, requester, dt_start, latest_tessellation_version: str, all_supported_cluster_data: list[dict], configuration: dict) -> list:
    async def locate_ips(requester):
        if requester is None:
            return list(set(await dask_client.compute(subscriber_dataframe["ip"])))
        else:
            return list(set(await dask_client.compute(subscriber_dataframe["ip"][subscriber_dataframe["name"] == str(requester)])))
    ...
    for fut in subscriber_futures:
        ...
        for k, v in subscriber.items():
            if k == "public_l0":
                for port in v:
                    layer = 0
                    request_futures.append(asyncio.create_task(check(dask_client, process_msg, requester, subscriber, layer, port, latest_tessellation_version, all_supported_cluster_data, history_dataframe, dt_start, configuration)))
            elif k == "public_l1":
                for port in v:
                    layer = 1
                    request_futures.append(asyncio.create_task(check(dask_client, process_msg, requester, subscriber, layer, port, latest_tessellation_version, all_supported_cluster_data, history_dataframe, dt_start, configuration)))
